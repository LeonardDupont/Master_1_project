function [extracted_fluorescence_sweeps] = fluorescence_alignment_HighPulse(filename,nb_sweeps)
%This function takes the whole fluorescence trace from the 2P imaging, cuts
%it in small pieces according to the HighPulse protocol waveform and gives back a
...
for region=1:x2-1
    ...
    for sweep=1:nb_sweeps
        ...
        Maxes(region) = max(Back_to_zero); %and store the max value here for the next round of the loop
    end
    ...
    for k=time_vector
        if 
            ...
            c=c+1;
        elseif k>4
            current_trace(c) = 0       ; %completely arbitrary
            c=c+1;
        elseif (4>k) && (k>2)
